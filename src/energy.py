...
class Energy(nn.Module):
    ...
    def forward(self):
        ...
        # boundary constraint
        # left = self.mesh[0, self.Q: - self.Q, 0] - self.source_mesh[0, self.Q: - self.Q, 0]
        # left = (left * (left > 0)) ** 2
        #
        # right = self.mesh[0, self.Q: - self.Q, - 1] - self.source_mesh[0, self.Q: - self.Q, - 1]
        # right = (right * (right < 0)) ** 2
        #
        # top = self.mesh[1, 0, self.Q: - self.Q] - self.source_mesh[1, 0, self.Q: - self.Q]
        # top = (top * (top > 0)) ** 2
        #
        # bottom = self.mesh[1,  - 1, self.Q: -self.Q] - self.source_mesh[1, - 1, self.Q: - self.Q]
        # bottom = (bottom * (bottom < 0)) ** 2



        left = self.mesh[0, self.Q: - self.Q, self.Q] - self.source_mesh[0, self.Q: - self.Q, self.Q]
        left = (left * (left > 0)) ** 2
        # left = left ** 2

        right = self.mesh[0, self.Q: - self.Q,  - self.Q - 1] - self.source_mesh[0, self.Q: - self.Q,  - self.Q - 1]
        right = (right * (right < 0)) ** 2
        # right = right ** 2

        top = self.mesh[1, self.Q, self.Q: - self.Q] - self.source_mesh[1, self.Q, self.Q: - self.Q]
        top = (top * (top > 0)) ** 2
        # top = top ** 2

        bottom = self.mesh[1,  - self.Q - 1, self.Q: -self.Q] - self.source_mesh[1,  - self.Q - 1, self.Q: - self.Q]
        bottom = (bottom * (bottom < 0)) ** 2
        # bottom = bottom ** 2
        ...
        # 全边界相等：
        # left = self.mesh[0, 0:-1, 0] - self.source_mesh[0, 0: -1, 0]
        # right = self.mesh[0, 0:-1, -1] - self.source_mesh[0, 0: -1, -1]
        # top = self.mesh[1, 0, 0:-1] - self.source_mesh[1, 0, 0:-1]
        # bottom = self.mesh[1, -1, 0:-1] - self.source_mesh[1, -1, 0:-1]
        # boundary_constraint += torch.square(left).mean() + torch.square(right).mean() + torch.square(top).mean() + torch.square(bottom).mean()
